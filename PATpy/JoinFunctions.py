...
def CreateWorkspace(path):
    """ This function will check if a .gbp workspace exists. If there is not one, the script will make one . The .gbp workspace is useful for working with csv data"""
...
def SetupPerilsEUWS(maptype):
    """ Moves the RMS MXD template and shapefiles from the GRM drive to the local drive. MXD location is hardcoded. Maptype options = 'CRESTA' or 'Postalcode'"""
...
def JoinSHP(jointable, csvjoinfield, shapefile, shpjoinfield, fields):
    ...
    arcpy.JoinField_management(shapefile, shpjoinfield,jointable, csvjoinfield, fields)
...
def CreateMapPercentChange(mxds,shplyr,mapfields):
    """This function will create maps for all mxds specified and all fields in the mapfields list. The symbology used is % change. If the symbology does not exist locally, this function will copy the necessary files from the network into the mxd\symbology folder. """
    ...
    destdir = "C:\Mapping_Project\MXDs\Symbology"
...
# def CreateMapDiffLC(mxds,shplyr,mapfields):
#         """This function will create maps for all mxds specified and all fields in the mapfields list. The symbology used is % change. If the symbology does not exist locally, this function will copy the necessary files from the network into the mxd\symbology folder. """
...
#     destdir = "C:\Mapping_Project\MXDs\Symbology"
#     if not os.path.exists("C:\Mapping_Project\MXDs\Symbology\DiffLC.lyr"):
#         for fl in glob(r"\\ca1ntap01\grm\PAT\ArcMappingTool\Symbology\DiffLC\\*"):
#             shutil.copy2(fl,destdir)
